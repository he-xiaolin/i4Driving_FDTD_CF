%% Pupil diameter – Male vs Female (rescaled + trimmed)
clc; clear; close all
...
meanPD_M=splitapply(@mean,all_work_M,findgroups(all_pid_M));
meanPD_F=splitapply(@mean,all_work_F,findgroups(all_pid_F));

figure
boxplot([meanPD_M; meanPD_F], ...
        [repelem({'Male'},numel(meanPD_M))' ; ...
         repelem({'Female'},numel(meanPD_F))' ], ...
        'Symbol','')
ylabel('Pupil diameter')
title('Pupil diameter – Male vs Female')
set(gca,'XLim',[0.5 2.5])

sd_M=std(meanPD_M); sd_F=std(meanPD_F);
[H,p,CI,stats]=ttest2(meanPD_M,meanPD_F);
fprintf('\nMale   mean = %.3f (SD = %.4f)  |  Female mean = %.3f (SD = %.4f)\n', ...
        mean(meanPD_M),sd_M,mean(meanPD_F),sd_F)
fprintf('t(%d) = %.2f,  p = %.4f\n\n',stats.df,stats.tstat,p)

if ~exist('./Figure','dir'), mkdir('./Figure'); end
exportgraphics(gcf,'./Figure/Pupil_Gender_Male_vs_Female_trimmed.png','Resolution',300)
