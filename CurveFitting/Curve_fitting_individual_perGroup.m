...
% - 新增：按 Age(A/B)、Gender(F/M)、Day/Night(D/N from filename) 对 “全部 h0_exp” 画箱线图
% - 新增：所有图导出保存到 ./curveFittingFigures/
% -------------------------------------------------------------------------
clc; clear; close all
...
%% ---- 添加分组标签：Age / Gender / DayNight（从文件名解析） ----
Age    = strings(nDrv,1);
Gender = strings(nDrv,1);
DN     = strings(nDrv,1);   % D=Day, N=Night, U=Unknown

for k = 1:nDrv
    % 1) Age & Gender（直接读字段；若是char，取第一个字符）
    if isfield(participants_CF(k),'ageGroup') && ~isempty(participants_CF(k).ageGroup)
        Age(k) = string(participants_CF(k).ageGroup(1));
    else
        Age(k) = missing;
    end
    if isfield(participants_CF(k),'genderGroup') && ~isempty(participants_CF(k).genderGroup)
        Gender(k) = string(participants_CF(k).genderGroup(1));
    else
        Gender(k) = missing;
    end
    % 2) Day/Night：从文件名里的 i4Driving_D / i4Driving_N 解析
    fn = participants_CF(k).filename;
    DN(k) = string(local_detectDN(fn));  % 返回 'D' / 'N' / 'U'
end

% 写入 FitTable（按固定顺序建为分类变量）
FitTable.Age      = categorical(upper(Age),   {'A','B'});
FitTable.Gender   = categorical(upper(Gender),{'F','M'});
FitTable.DayNight = categorical(upper(DN),    {'D','N','U'});  % U=未识别
...
%% ============ 按分组画箱线图（全部 h0_exp，不筛 R²） =====================
h0_all = FitTable.h0_exp;   % 全部被试的 h_exp

% --- Age: A / B
fAge = figure;%('units','normalized','outerposition',[0 0 0.45 0.5]);
boxplot(h0_all, removecats(FitTable.Age), 'Notch','off','Whisker',1.5);
set(gca,'YScale','log'); 
% grid on
ylabel('h_{exp}  [s]'); xlabel('Age group (A/B)')
title('h_0 by Age (all drivers)')
[statsAge, testsAge] = addStatsAndTestsToBox(gca, h0_all, FitTable.Age);   % ★ 加这行
exportgraphics(fAge, fullfile(outdir,'Boxplot_h0_by_Age.jpg'), 'Resolution',300);

% --- Gender: F / M
fGen = figure;%('units','normalized','outerposition',[0 0 0.45 0.5]);
boxplot(h0_all, removecats(FitTable.Gender), 'Notch','off','Whisker',1.5);
set(gca,'YScale','log'); 
% grid on
ylabel('h_{exp}  [s]'); xlabel('Gender (F/M)')
title('h_0 by Gender (all drivers)')
[statsGen, testsGen] = addStatsAndTestsToBox(gca, h0_all, FitTable.Gender); % ★ 加这行
exportgraphics(fGen, fullfile(outdir,'Boxplot_h0_by_Gender.jpg'), 'Resolution',300);

% --- Day vs Night: 从文件名解析的 D / N（去掉未识别 U）
dnCats = removecats(FitTable.DayNight);
isDN   = dnCats ~= 'U' & ~ismissing(dnCats);
fDN = figure;%('units','normalized','outerposition',[0 0 0.45 0.5]);
boxplot(h0_all(isDN), dnCats(isDN), 'Notch','off','Whisker',1.5);
set(gca,'YScale','log'); 
% grid on
ylabel('h_{exp}  [s]'); xlabel('Session (D=Day, N=Night)')
title('h_0 by Day vs Night (all drivers)')
[statsDN, testsDN] = addStatsAndTestsToBox(gca, h0_all(isDN), dnCats(isDN)); % ★ 加这行
exportgraphics(fDN, fullfile(outdir,'Boxplot_h0_by_DayNight.jpg'), 'Resolution',300);
...
% 全局散点 + 两模型曲线
fGlobal = figure('units','normalized','outerposition',[0 0 1 1]);
...
exportgraphics(fGlobal, fullfile(outdir,'Global_scatter_and_fits.jpg'), 'Resolution',300);

%% =============================  兜底：保存全部图  =========================
% （再统一导出一遍防漏存；文件名为 Figure_1.jpg, Figure_2.jpg, ...）
allFigs = findall(0,'Type','figure');
for i = 1:length(allFigs)
    f = ancestor(allFigs(i),'figure');
    if ~isempty(f)
        filename = sprintf('%s/Figure_%d.jpg', outdir, i);
...
function tag = local_detectDN(fn)
% 从文件名提取 D/N：匹配 'i4Driving_D' 或 'i4Driving_N'
% 返回 'D' / 'N' / 'U'(未识别)
    tag = 'U';
    if ~ischar(fn) && ~isstring(fn), return; end
    fn = char(fn);
    tok = regexp(fn,'i4Driving_([DN])','tokens','once','ignorecase');
    if ~isempty(tok)
        tag = upper(tok{1});
        return
    end
    % 兜底：宽松判断
    if contains(fn,'i4Driving_D','IgnoreCase',true), tag='D'; return; end
    if contains(fn,'i4Driving_N','IgnoreCase',true), tag='N'; return; end
end

% function [statsTbl, testsTbl] = addStatsAndTestsToBox(ax, y, grp)
% % 计算每组 mean/SD/IQR，并进行双组显著性检验；把结果写到图上
%     % 清洗
%     m = ~ismissing(grp);
%     y   = y(m);
%     grp = removecats(grp(m));
%     % cats = categories(grp);
% cats = string(categories(grp));  % 强制转成 string，避免 char->categorical 报错
% 
%     % 逐组统计
%     G = numel(cats);
%     % statsTbl = table('Size',[G 8], 'VariableTypes', ...
%     %     {'categorical','double','double','double','double','double','double','double'}, ...
%     %     'VariableNames',{'Group','n','Mean','SD','Median','Q1','Q3','IQR'});
%     statsTbl = table('Size',[G 8], 'VariableTypes', ...
...
%     for i = 1:G
%         yi = y(grp==cats{i});
%         q  = prctile(yi,[25 50 75]);
%         statsTbl.Group(i)  = categorical(cats{i});
%         statsTbl.n(i)      = numel(yi);
%         statsTbl.Mean(i)   = mean(yi,'omitnan');
%         statsTbl.SD(i)     = std(yi,0,'omitnan');
%         statsTbl.Median(i) = q(2);
%         statsTbl.Q1(i)     = q(1);
%         statsTbl.Q3(i)     = q(3);
%         statsTbl.IQR(i)    = q(3)-q(1);
%     end
% 
%     % 显著性检验（仅双组时给出）
%     testsTbl = table;
...
%         x2 = y(grp==cats{2});
%         % 非参数检验（默认报告）
%         [p_rs,~,stats_rs] = ranksum(x1,x2);
%         % 对数域 Welch t-test（正偏分布更稳健）
%         [~,p_t] = ttest2(log(x1),log(x2),'Vartype','unequal');
%         d = cohen_d(log(x1),log(x2));        % 效果量（对数域）
%         delta = cliffs_delta(x1,x2);         % Cliff's delta（非参数效果量）
%         testsTbl = table(p_rs,p_t,d,delta, 'VariableNames', ...
%                          {'p_ranksum','p_t_log','cohen_d_log','cliffs_delta'});
% 
%         star = pstars(p_rs);
%         info = sprintf('%s vs %s: ranksum p=%.3g %s | Welch t(log) p=%.3g, d=%.2f | Cliff''s \\delta=%.2f', ...
%                        cats{1}, cats{2}, p_rs, star, p_t, d, delta);
%     else
%         info = 'More than two groups – no pairwise test shown.';
%     end
% 
%     % 把统计与检验写到图上（左上角）
%     lines = strings(G+1,1);
%     for i=1:G
%         lines(i) = sprintf('%s (n=%d): mean=%.3g, SD=%.3g, IQR=[%.3g, %.3g] (%.3g)', ...
%             string(statsTbl.Group(i)), statsTbl.n(i), statsTbl.Mean(i), statsTbl.SD(i), ...
%             statsTbl.Q1(i), statsTbl.Q3(i), statsTbl.IQR(i));
%     end
%     lines(end) = info;
% 
%     text(ax, 0.01, 0.99, strjoin(lines,newline), 'Units','normalized', ...
%         'VerticalAlignment','top','FontSize',9,'FontName','FixedWidth', ...
%         'BackgroundColor',[1 1 1], 'Margin',3);
% 
%     % 同时在命令行打印
%     disp(statsTbl); 
%     if ~isempty(testsTbl), disp(testsTbl); end
% end

% function [statsTbl, testsTbl] = addStatsAndTestsToBox(ax, y, grp)
% % addStatsAndTestsToBox(ax, y, grp)
% % 在箱线图上方显示每组统计量（mean/SD/IQR），并进行显著性检验。
% % - 双组：Mann–Whitney（ranksum）、Welch t-test(对数域)，效果量：Cohen's d(对数域) & Cliff's delta
% % - 会把结果叠加到图上，并返回 statsTbl/testsTbl，同时把表打印到命令行。
% %
% % 输入:
% %   ax  : 目标轴句柄（当前箱线图所在轴）
% %   y   : 数值向量（如 h0_exp）
% %   grp : 分组变量（categorical/string/char/cellstr 均可；会转为 categorical）
% %
% % 输出:
% %   statsTbl : 每组统计量表 (Group,n,Mean,SD,Median,Q1,Q3,IQR)
% %   testsTbl : 双组时的检验结果表 (p_ranksum,p_t_log,cohen_d_log,cliffs_delta)
% 
%     if nargin<1 || isempty(ax), ax = gca; end
%     y   = y(:);
%     grp = grp(:);
% 
%     % 统一为 categorical，移除缺失类别；同步清洗 y 的 NaN/Inf
%     if ~iscategorical(grp)
%         try
%             grp = categorical(grp);
%         catch
%             grp = categorical(string(grp));
%         end
%     end
%     m = ~isnan(y) & ~isinf(y) & ~ismissing(grp);
%     y   = y(m);
%     grp = removecats(grp(m));
% 
%     cats = string(categories(grp));    % 用 string 避免 char→categorical 报错
%     G    = numel(cats);
% 
%     % -------- 每组统计量 --------
%     statsTbl = table('Size',[G 8], 'VariableTypes', ...
...
% 
%     for i = 1:G
%         yi = y(grp == cats(i));                  % 支持 categorical == string
%         q  = prctile(yi,[25 50 75]);
%         statsTbl.Group(i)  = cats(i);
%         statsTbl.n(i)      = numel(yi);
%         statsTbl.Mean(i)   = mean(yi,'omitnan');
%         statsTbl.SD(i)     = std(yi,0,'omitnan');
%         statsTbl.Median(i) = q(2);
...
%     % -------- 显著性检验（仅双组） --------
%     testsTbl = table;
%     infoLine = "More than two groups – no pairwise test shown.";
%     if G == 2
%         x1 = y(grp == cats(1));
%         x2 = y(grp == cats(2));
% 
%         % 非参数 Mann–Whitney（ranksum）
%         [p_rs,~,~] = ranksum(x1, x2);
% 
%         % 对数域 Welch t-test（h0>0，若有<=0则略过 t 检）
%         useT = all(x1>0) && all(x2>0);
%         if useT
%             [~,p_t] = ttest2(log(x1), log(x2), 'Vartype','unequal');
%             dlog   = cohen_d(log(x1), log(x2));     % Cohen's d（log 域）
%         else
%             p_t  = NaN; dlog = NaN;
%         end
% 
%         % Cliff's delta（非参数效果量）
%         delta = cliffs_delta(x1, x2);
% 
%         testsTbl = table(p_rs, p_t, dlog, delta, ...
%             'VariableNames', {'p_ranksum','p_t_log','cohen_d_log','cliffs_delta'});
% 
%         infoLine = sprintf('%s vs %s: ranksum p=%.3g %s | Welch t(log) p=%.3g, d=%.2f | Cliff''s \\delta=%.2f', ...
%                            char(cats(1)), char(cats(2)), p_rs, pstars(p_rs), p_t, dlog, delta);
% 
%         % 在图上画显著性括号 + 星号
%         try
%             hold(ax,'on');
...
%             yl = ylim(ax);
%             ymax = max(y);  ymin = min(y(y>0)); if isempty(ymin), ymin = yl(1); end
%             if strcmpi(yscale,'log')
%                 y1 = ymax * 1.15;
%                 y2 = ymax * 1.25;
%                 if y2 >= yl(2), ylim(ax,[yl(1) y2*1.05]); end
%             else
%                 r  = range([y; yl(:)]);
%                 if r==0, r = max(yl(2)-yl(1), 1); end
%                 y1 = yl(2) - 0.05*r;
%                 y2 = yl(2) - 0.02*r;
%             end
%             plot(ax,[1 1 2 2],[y1 y2 y2 y1],'k-','LineWidth',1);
%             text(ax, 1.5, y2, pstars(p_rs), 'HorizontalAlignment','center', ...
...
%     for i=1:G
%         lines(i) = sprintf('%s (n=%d): mean=%.3g, SD=%.3g, IQR=[%.3g, %.3g] (%.3g)', ...
%             char(statsTbl.Group(i)), statsTbl.n(i), statsTbl.Mean(i), statsTbl.SD(i), ...
%             statsTbl.Q1(i), statsTbl.Q3(i), statsTbl.IQR(i));
%     end
%     lines(end) = infoLine;
% 
...
%         sx2 = var(x,1,'omitnan'); sy2 = var(y,1,'omitnan');  % MLE 方差
%         sp  = sqrt(((nx-1)*sx2 + (ny-1)*sy2) / max(1,(nx+ny-2)));
%         d   = (mean(x,'omitnan') - mean(y,'omitnan')) / max(sp,eps);
%     end
% 
%     function delta = cliffs_delta(x,y)
%         x = x(:)'; y = y(:)';
%         [X,Y] = ndgrid(x,y);
%         delta = (sum(X(:)>Y(:)) - sum(X(:)<Y(:))) / numel(X);
%     end
% end
...
function d = cohen_d(x,y)
% 独立样本 Cohen's d（Hedges 修正可按需加）
    nx = numel(x); ny = numel(y);
    sx2 = var(x,1); sy2 = var(y,1);                     % MLE 方差
    sp  = sqrt(((nx-1)*sx2 + (ny-1)*sy2) / max(1,(nx+ny-2)));
    d   = (mean(x)-mean(y)) / max(sp,eps);
end
...
function [statsTbl, testsTbl] = addStatsAndTestsToBox(~, y, grp)
% addStatsAndTestsToBox(~, y, grp)
% 仅计算并返回每组统计量与显著性检验结果，不在图上标注。
% 输出：
%   statsTbl: Group, n, Mean, SD, Median, Q1, Q3, IQR
%   testsTbl: (仅双组) p_ranksum, p_t_log, cohen_d_log, cliffs_delta
    ...
    if G == 2
        ...
        % 对数域 Welch t-test（仅当全 >0 时）
        useT = all(x1>0) && all(x2>0);
        if useT
            [~,p_t] = ttest2(log(x1), log(x2), 'Vartype','unequal');
            dlog   = cohen_d(log(x1), log(x2));
        else
            p_t  = NaN; 
            dlog = NaN;
        end
